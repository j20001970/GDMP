 ...
 */
public class GDMPCameraHelper extends CameraHelper {
  /**
   ...
   * image analysis use case from {@link CameraX}.
   *
   * @param analyzer Interface for analyzing images.
   * @param targetSize the preview size to use. If set to {@code null}, the helper will default to
   *     1280 * 720.
   */
  public void startCamera(
  ...
  /**
   * Initializes the camera and sets it up for accessing frames. This constructor also enables the
   * image analysis and capture use case from {@link CameraX}.
   *
   * @param analyzer Interface for analyzing images.
   * @param imageCaptureBuilder Builder for an {@link ImageCapture}, this builder must contain the
   *     desired configuration options for the image capture being build (e.g. target resolution).
  ...
  public void startCamera(
    ...
    cameraProviderFuture.addListener(
        () -> {
          ...
          if (analyzer != null && imageCaptureBuilder != null) {
            imageAnalysis = new ImageAnalysis.Builder()
                .setBackpressureStrategy(ImageAnalysis.STRATEGY_KEEP_ONLY_LATEST)
                .setTargetResolution(rotatedSize)
                .build();
            imageAnalysis.setAnalyzer(renderExecutor, analyzer);
          ...
          } else if (imageCaptureBuilder != null) {
            imageCapture = imageCaptureBuilder.build();
            camera =
                cameraProvider.bindToLifecycle(
   ...
   * method works when {@link GDMPCameraHelper#startCamera(Activity, ImageCapture.Builder,
